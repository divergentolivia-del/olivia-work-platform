...
function bindEvents() {
  ...
  const drawerList = document.getElementById("mobileDrawerList");
  const drawerClose = document.getElementById("mobileDrawerClose");
  const drawerMask = drawer ? drawer.querySelector(".mobile-drawer-mask") : null;
  ...
  function openMobileDrawer() {
    ...
    const aiIdx = orderedModules.findIndex((m) => m.id === "ai-center");
    const dashIdx = orderedModules.findIndex((m) => m.id === "dashboard");
    if (aiIdx > -1 && dashIdx > -1 && aiIdx !== dashIdx + 1) {
      const [aiMod] = orderedModules.splice(aiIdx, 1);
      orderedModules.splice(dashIdx + 1, 0, aiMod);
    }
    drawerList.innerHTML = orderedModules.map((mod) =>
      `<button class="mobile-drawer-item ${mod.id === state.moduleId ? "active" : ""}" data-module="${mod.id}">
        <span class="nav-icon">${mod.icon}</span>
        <span>${esc(mod.title)}</span>
      </button>`
    ).join("");
    // 绑定点击
    drawerList.querySelectorAll(".mobile-drawer-item").forEach((item) => {
      item.onclick = () => {
    ...
    // 底部按钮
    drawer.querySelectorAll("[data-footer]").forEach((btn) => {
      btn.onclick = () => {
        const action = btn.dataset.footer;
        closeMobileDrawer();
        if (action === "recycle") showToast("回收站功能开发中");
  ...
  if (bottomNavMore) bottomNavMore.onclick = openMobileDrawer;
  if (drawerClose) drawerClose.onclick = closeMobileDrawer;
  if (drawerMask) drawerMask.onclick = closeMobileDrawer;
...
function render() {
  ...
  // "更多"按钮在当前模块不在底部导航的4个快捷入口时高亮
  const bottomNavModules = ["dashboard", "project", "ai-center", "learning"];
  const moreBtn = document.getElementById("bottomNavMore");
  if (moreBtn) moreBtn.classList.toggle("active", !bottomNavModules.includes(state.moduleId));
}
